    ...
    def test_offers_for_customer(self):
        ...
        Подходы:
            ...
            2) удалить те офферы что которых нет в CSV и добавить те что есть в CSV(Обновлять не имеет сыска так как оффер вобщем то не сущность).
                Удалять - занчит указать что в момент загрузки они стали не активны(если через несколько выгрузок он станет активным то мы должны видеть это)
                Что делать если изменились остатки по офферу? Или цена?
                Что делать если заказ оформился по одному офферу, но еще не прошел по всем статусам, а оффер уже удален?
                    должны ли мы обращаться к проверки существования офера после создания заказа или нет?
                        закупка в рамках одного заказ идет от дух поставщиков
                            от первого мы должны закупить на 5 дней раньше чем от второго.
                            намомент формирования заказа у первого поставщика было то чо указано в заказе по его номиенклатуре
                            увторого было все по его позициям.
                            но через 5 дней после того как первый поставил на наш склад.
                            пытаемся сделать перемещение с второго поставщика но
                            второй сказал чо у него нет данного офера больше(изменилась цена. остаток...)
                                или просто подтвердили резерв не правильно.
