...
class PlanFactEvent(models.Model):
    ...
    Для выполенния заказа нужно:
        Запланировать действие по выдаче данного заказа.
        Далее домать до времени выдачи(выполнить все запланированные операции в системе) и попробовать осуществить выдачу.
            Вопрос, как проверить что данная выдача не зааффектит те операции что будут после?
                Можно проматать все действия до конца.
                    Если все хорошо стоит учесть действия с этим product_guid для дальнейшего его "резрвирования" не использования для продажи в других заказах(Но раз уже действие по выдаче клиенту есть то не продавать).
                    Если плохо нужно обеспечить такое состояние системы при котором данный заказ можно выдать.
                        для этого нужно переместить товар откукдато на point выдачи и не сламать перемещением обязательные к испольнению действия.
                            Впринципе обязательных к исполнению нет, есть приортитетные. Просто обеспечить заказ клиента это важнее чем переместить под достатвьчность,
                                но если за товар спорят два клиента то какой из заказов более приороритеный
                            А также надо снаачало попцтаться обеспечить все заказы, всеми возмодными способами,затесм обеспечить  перемещения.
                                но так как любое действие делается под определенную задачу. То под заказа задачи более приоритетны.
                            Если все заказы обесепчить не получаетсяч то лучше обеспечить сначало более важные заказы.

    Аксиомы:
        Система меняет сове сотояние со временим.
        У системы может быть много состояний в будщем, надо выбрать из всех боле оптимальное и начать двигаться в его сторону.
        Можем выработать концепуию когда наличие заказа покупателя на товар на 100% бронирует его за покупателем.
        Но есть момент, выкупаемость у пред закзаов очень низкая. Поэтому 100% бронировать за клиентами с низкой выкупаемостью в дни старта продаж это очень растачительно.
            Если бы был один магазин и склад на нем то клиентов можно было бы отоварить всех кто пришел в порядке живой очереди.
                Но двать обещеание и не выполнять плохой подход. Следует давать только те обещания что готовы выполнить. А занчит ужесточить выбор тех кому давать обещания.
                Давать разные обещания:
                    Пред заказ + передоплата = 100% товар (тоже не можем обеспечить так как может товар не прияти к нам)
                    Пред заказ + кредит = 99% товар.
                    Пред заказ + наличные = будете иметь приортитетную выдачу не обязательно товар вам достанется.

    Система фиксации событий:
    Действия бывают:
        Обратимые перемещение товаров по складам
        Не обратимые реализация товара покупателям
    Какие возможны действия:
        Перевести с склада на склад
            Наполнение под достаточность
            Вывоз брака
            Пересортировка
        Собрать товар в заказ(упаковать)
        Погрузить в транспортное средство.
        Получить деньги отпокупателя за ...
        Выдать покупателю.
        Доставить курьером покупателю.
        Выдать деньги покупателю в связи с возвратом(брак, 2 недели не устроил, ...)
        Принять от покупателя(возврат)

    Что делать если по товару уже есть действия?
        Если эти действия не связанные с реализацие товаров покупателю то их нужно выполнять в связи с приортетом.
            Что если товар прише в магазин под ремонт,
                затем перевести в тех центр,
                починить,
                вирнуть в магазин,
                выдать покупателю.
            Не получится так что действие выдать покупателю залочит все мезанизмы поперемещению в ремонт и обратно?

    Множно описать несколько кейсов:
        1. Товар одна штука на складе.

        2.
            Начальные условия:
                Текущая дата 19.10.2019 10:25.
                Покупателю нужен product_1.
                Товар который нужен покупателю есть на point_1 и point_2 точки выдачи по цене price_1 уже сейчас,
                    и через 4 дня его могут доставть с point_1 на point_3.
                Покупатель хочет получить на point_2.
            Реализация:
                Покупатель оставляет на сайте намерение приобрести товар по цене price_1
                    в точке выдачи point_2 22.10.2019 с резервом 24 часа(может прийти с 22.10.2019 14:00 до 23.10.2019 13:59)
                    и оплатой наличными на месте и в момент получения товвара ~ "заказ".
                Чтобы обеспечить намерение покупателя нам нужно чтобы на точке point_2 был товар product_1 в промежутке с 22.10.2019 14:00 до 23.10.2019 13:59
                    и не уходил под доругие заказы.
                    также должна работь касса и принимать наличные и на месте должен быть продавец.
                Товар есть на точке и оплат при получении то нужно завести(запланировать) действия.
                    Получить оплату на точке выдачи.
                        Не известно точное время события, а только диапазон в который оно возмодно случиться.
                            Плюс оно растянуто во времени(оплат может занять 5 минут)
                    Выдать товар после получения оплаты.
                    Пробить чек.
                Покупатель приходит на точк 22.10.2019 15:49.
                    Продавец открывает задачи связанные с заказом,
                        принимает оплату.
                        выдает товар product_1.
                        пробивает чек.
                    В учетной системе в 22.10.2019 16:01 должны появиться две записи:
                        1. списании товара с отаков магазина основанием будет чек.
                        2. принятие наличных денег в кассу магазина основанием для которго будет также чек.

        3.
            Начальные условия:
                Такие же как в пункте 2. НО,
                    Покупатель хочет получить на point_3.
            Реализация:
                Покупатель оставляет на сайте намерение приобрести товар по цене price_1
                    в точке выдачи point_3 25.10.2019 с резервом 24 часа(может прийти с 25.10.2019 14:00 до 26.10.2019 13:59)
                    и оплатой наличными на месте и в момент получения товвара ~ "заказ".
                Чтобы обеспечить намерение покупателя нам нужно чтобы на точке point_3 был товар product_1 в промежутке с 25.10.2019 14:00 до 26.10.2019 13:59
                    и не уходил под доругие заказы.
                    также должна работь касса и принимать наличные и на месте должен быть продавец.
                Товар нет на точке, значит нужно его как то туда доставить.
                    Система знате что товар есть на point_1 и point_2, и под достаточность он не едет на point_3.
                        Но может доставить с point_2 за 4 дня, перевозчиком courier_1 котрый ездит каждый день.
                    Тогда фоормируем задения:
                        Выдать product_1 на складе point_2 курьеру courier_1 в мемент времени 19.10.2019 13:00 (скорее всегот точное время не известно, известен диапазон)
                        courier_1 следует перевезти product_1 из point_2 в point_3 забрав его в какойто момент помежутка времни с 19.10.2019 10:25. до 25.10.2019 12:59
                            но чтобы на point_3 он появился за час до выдачи клиенту 25.10.2019 12:59
                                чтобы на точке point_3 была возмодность принять товар от курьера и положить на полку вдачи(обработать поставку, пропикать штризх кода, осмотреть упаковку, ...)
                        point_3 нужно принять товар от курьера courier_1 который приедет с 19.10.2019 10:25 до 25.10.2019 12:59
                Так как оплата при получении то нужно завести(запланировать) действия.
                    Получить оплату на точке выдачи.
                        Не известно точное время события, а только диапазон в который оно возмодно случиться.
                            Плюс оно растянуто во времени(оплат может занять 5 минут)
                Выдать товар после получения оплаты.
                ...
                Курьер приеха 19.10.2019 14:33 на point_2 за product_1. (курьер о заказе клиента ничего не знает!)
                    точка отгрзила его курьеру
                    курьер принял к перевозке.

                в 19.10.2019 15:07 точка отпраила иформацию в копьютер точки(провила документы) товар отгружен.

                24.10.2019 9:51 курьер приезхал на точку point_3.
                    курьер отгрзила product_1 на точку
                    точка приняла product_1 на остатки.

                24.10.2019 18:30 точка првела документы по приянтию в компьютер точки(и может тепрь этот продукт продавать)
                24.10.2019 18:45 точка отправила данные в центральную систему отом что приняла поставку.
                    теоретически провести приемный документ можно даже после пробития чека, если сисетма пробития чеков простой апарат не учитвающий остатки на точке.
                    Есть договоренность что точка или склад или курьер должны внести информацию о проделанных действиях в центральню систему точно в течении 14 дней.

                Покупатель приходит на точку 26.10.2019 9:12.
                    Продавец открывает задачи связанные с заказом,
                        принимает оплату.
                        выдает товар.
                        пробивает чек.
                    В учетной системе в 26.10.2019 10:17 должны появиться две записи:
                        1. списании товара с отаков магазина основанием будет чек.
                        2. принятие наличных денег в кассу магазина основанием для которго будет также чек.

                27.10.2019 10:01 курьер отправлет данные по принятию к перевозке груза от 19.10.2019.
                27.10.2019 10:07 курьер отправлет данные по выдаче с перевозки груза от 24.10.2019.

                Итого: только 27.10.2019 10:07 система вошла в консистентное состояние.
                    До этого моемента товар фактически пропадал из системы на момент первозки.

                    Можем ли мы в финальных отчетах использовать инфу о планируемых действиях?
                        ответа будет два.
                            1. если мы соблюдаем консистентность, не планируем несколько не совмещенных действий
                                (одну и туже штуку в один день с одного склад отправит в 2 разных места - физичекси ен возмодно)
                                то да, можем.
                            2. если консистентность не соблюдаем то нельзя(мы будем учитывать эту штуку перевозимую одновремнно в два разных склада дважды)

                    Получается факт всегда будет отставать от реальности так как документы попадают в систему в разные моменты времени
                        может быть документ созданный раньше придет в систему позже и финальный учет будет не коерктным.

                    И полан также не льзя использовать так как запаленированные действия могут быть взаимо исключающими и одно заблочит другое.

                    Как тогда понять текущее состояние системы?
                        Возможно нужно учитывать только те документы которые идут подряд.
                            Но идентификация документов в магазинах своя, в центральной системе своя.

                    Есть планируемы действия которые:
                        1. переходят в стостояние совергшенное (данное действие совершилось, что деалть если оно совершилось частично?)
                        2. Есть действия которые не совершились
                        3. Действия которые совершилсь но инфа по ним в центральную систему не пришла.
                            вопрос как их отлечить от несовершенных?
                            надо ли их отличать от не совершенны?
                                надо чтобы искать документы по ним и совершать действия, иначе система будет показывать не верные данные.
                                    Пример, перевоз с точки на точку. точка вторая уже приняла товар и сообщила об этом а первая еще не сообщила о списании, а значит товара в ситстеме стало больше!

                    Если бы задания отдавал только цен тральная система то это было бы проще.
                        Но точка сама может продать товар покупателю зашедшему и тогда центральная система узанет только факт продажи.
                        Или точки две договорятся и первезут часть товара содной на другую. И могут не сообщеить центральной системе.
                            В этот момент центральная система будет планировать пересортировку товара на точках реализации с некоректными остатками.

                    Желательно иметь механизм опроса точек, складов, курьеров, по совершенным действиям. Чтобы получать все соверешшные действия на данный моемент.
                        Но курьер сточка склад - это электронные системы учета - и в них информации тоже может не быть,
                            а занчит нужно невилировать человеческий фактор или автоматизировать все процессы(пользователи ходят с компьютерами и фиксируют все сразу)
                            а мини компы потом с минимаольным лагом отправляют все в точку скалад или курьеру - откуда уже уходит инфа в центральную систему
                    Однако технические проблемы в связи итзадержки не кто не отменял.
                        То что хотят онлайн будет возмоджно только при 100% копмпьбтеризации и 24 на 7 связи с центральным сервером.
                        Т.е. все действия попадают в центральную систему без задрежек. Но если центральная система стоит. То вся комапния стоит!

            4. Корпоративный клиент.
                ...

    Решить текущие задачи(проблемы)

    1. Система магазина, не зависит от системы центральной.
    2. Событие в системе магазина происходит, а центральная узнает через некоторое время(или возможно канал связи упал)
        система магазина должна уметь работать независимо.

    Онлайн остаки + Перемещения(то что едет в магазин с сроком попадания в магазин).
        Показать что товар есть в наличии.
        Провести инвенторизацию сколько штук.
        Давать возможность клиенту на сайте оформить заказ с тех складов где есть позиции из заказа.
            Сформировать задание на премещение товаров в точку компоновки
                Затем переместить готовую упакову в точку выдачи
            Сформировать задание на принятие денег в точке выдачи
                или получить оплату онлайн и указать в закаще что средстав поступили
            Сформировать задание выдача клиенту закака в момент повяления клиента на точке.

            Т.е.
                1. предоставить возможность перевести(налом, картой, безналом, кредитом) деньги компании(сегодня, завтра, 22.10.2020, ...)
                2. предоставить возможность получить товар на указанной точек клиенту(переместить на эту точку достаточное количество товаров)
                Обоснованием для данных действий будет заказ.

    Что фиксируется в фактах:
        Те документы которые несут правовое подтверждение того что товар в собственности (у компании это торг 12 / у покупателя чек), основанием для появления денег в кассе магазина является пробитый чек.

    Для оплатой наличными чтобы не долго не подвешивать товар в резерве необходимо ограничить время полчения товара.
        Пример - если вы вбрали оплата наличными то вам надо притий на точку в течении 3 часов, иначе бронь снимится.
            если картой, то 5 дня с момент озвученного получения.

    Что делать если склад на инвенторизации и  не принимает входящие поставки, и не отправляет?

    Амазон множество доставок в рамках одного заказа.

    Для корпоративныз возможно несколькот оплат в рпамках одного заказа.
    """
    datetime_create = models.DateTimeField(u'дата и время создание', auto_now_add=True)
